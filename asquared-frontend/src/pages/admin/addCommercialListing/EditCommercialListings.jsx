...
const EditCommercialListing = () => {
  ...
  const fetchListingData = async (propertyId) => {
    try {
      const response = await axiosInstance.get(`http://localhost:5000/api/commercialListings/${propertyId}`);
      const { data } = response;
      setListingData(data);
    } catch (error) {
  ...
  const handleUpdateListing = async () => {
    try {
      await axios.put(`http://localhost:5000/api/commercialListings/${propertyId}`, listingData);
  ...
  const handleImageUpload = async () => {
    ...
    try {
      await axios.post(`http://localhost:5000/api/commercialListings/upload/${propertyId}`, formData, {
        headers: {
  ...
  return (
    <div className="main-container">
      <div className='button-edit-commercial'>
        <Link to="/admin">
          <button className='go-to-admin' type="button">Go to Admin</button>
        </Link>
    
      </div>
      <h2>Edit Commercial Listing</h2>
...
export default EditCommercialListing;
