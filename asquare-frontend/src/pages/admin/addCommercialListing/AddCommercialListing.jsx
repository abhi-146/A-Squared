...
const AddCommercialListing = () => {
  ...
  const handleAddOrUpdateListing = async () => {
    try {
        // Add new listing
        const response = await axiosInstance.post('http://localhost:5000/api/commercialListings/', listingData);
  ...
  const handleUploadImages = async () => {
    try {
      ...
      const response = await apiInstance.post(`commercialListings/upload/${uploadedId}`, formData, {
        headers: {
