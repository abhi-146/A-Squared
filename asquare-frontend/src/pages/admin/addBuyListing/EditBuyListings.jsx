...
const EditBuyListing = () => {
  ...
  const fetchListingData = async (propertyId) => {
    try {
      const response = await axiosInstance.get(`http://localhost:5000/api/buyListings/${propertyId}`);
      const { data } = response;
      setListingData(data);
    } catch (error) {
  ...
  const handleUpdateListing = async () => {
    try {
      await axiosInstance.put(`/buyListings/${propertyId}`, listingData);
  ...
  const handleImageUpload = async () => {
    ...
    try {
      await axiosInstance.post(`/buylistings/upload/${propertyId}`, formData, {
        headers: {
